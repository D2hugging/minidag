...
int main(int argc, char** argv) {
  ...
  for (const auto& dag_conf : app_config.dags) {
    try {
      ...
      std::cerr << "Failed to build DAG '" << dag_conf.name
                << "': " << e.what() << std::endl;
      return 1;
    }
  }
  ...
  svr.Post(R"(/api/v1/dags/([^/]+)/run)",
           [&manager](const httplib::Request& req, httplib::Response& res) {
             ...
             if (!manager.HasDag(dag_name)) {
               res.status = 404;
               res.set_content(
                   json{{"error", "Unknown DAG: " + dag_name}}.dump(),
                   "application/json");
               return;
             }
             ...
             try {
               ...
               res.set_content(
                   json{{"error",
                         std::string("Invalid request fields: ") + e.what()}}
                       .dump(),
                   "application/json");
               return;
             }
             ...
             try {
               ...
               if (future.wait_for(std::chrono::milliseconds(timeout_ms)) ==
                 ...
                 res.status = 504;
                 res.set_content(
                     json{{"error", "DAG execution timed out"},
                          {"dag", dag_name},
                          {"total_us", total_us}}
                         .dump(),
                     "application/json");
               ...
               res.status = 500;
               res.set_content(
                   json{{"error", std::string("DAG execution failed: ") +
                                      e.what()},
                        {"dag", dag_name},
                        {"total_us", total_us}}
                       .dump(),
